...
- If you want to be able to change the Circuitpython storage filesystem from Readonly (standard) to Writeable: rename the file boot_pico.py to boot.py. Then you can change the mode by pushing the \<BUTTON 1\> (= UP button) at the moment you press the Reset button on the board. My experience is that you have to perform another reset of the PicoW to see the new filesystem state in the opening text.
